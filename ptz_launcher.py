...
# 优先使用环境变量，便于在未改 yaml 的情况下指向本机 Isaac Sim 的 python.sh
PYTHON_SH     = os.environ.get("ISAAC_SIM_PYTHON") or cfg.get(
...
_TILT_MIN = -90.0
_TILT_MAX = 30.0
# ONVIF tilt 归一化 ↔ 度（与 docs/LOCAL_WORKTREE_OVERRIDES.md 一致，非远程仿射公式）
_TILT_SCALE = 90.0
...
def _norm_to_tilt(norm: float) -> float:
    return _clamp(-_clamp(norm, -1.0, 1.0) * _TILT_SCALE, _TILT_MIN, _TILT_MAX)


def _tilt_to_norm(tilt_deg: float) -> float:
    return -_clamp(tilt_deg, _TILT_MIN, _TILT_MAX) / _TILT_SCALE
...
def _flv_broadcaster() -> None:
    ...
    while True:
        if _isaac_state != "running" or not _port_in_use(_RTSP_PORT):
            time.sleep(0.5)
            continue
        ...
        cmd = [
            ...
            "-probesize", "32",
            "-rtsp_transport", "tcp",
            "-i", f"rtsp://localhost:{_RTSP_PORT}/ptz_cam",
            "-c:v", "copy",
            "-an",
            "-f", "flv", "pipe:1",
        ]
...
def main():
    ...
    print(
        f"[PTZ-Launcher] Isaac 子进程解释器：{PYTHON_SH!r}"
        + ("（环境变量 ISAAC_SIM_PYTHON）" if os.environ.get("ISAAC_SIM_PYTHON") else "（ptz_config.yaml 的 python_sh）")
    )
